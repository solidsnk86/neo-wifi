...
const locations = [
  ...
  { name: "Tierra del F." + " " + getCountryFlag("AR"), antennas: 4 },
  ...
  { name: "Los Angeles" + " " + getCountryFlag("US"), antennas: 15 },
  { name: "New York" + " " + getCountryFlag("US"), antennas: 15 },
];
      ...
      <aside className="flex gap-6 relative" id="marquee">
        <CloneElements>
          {locations.map(({ name, antennas }) => (
            <div
              key={name}
              className="flex items-center gap-4 p-4 w-[280px] transition hover:scale-[1.05] cursor-default group"
            >
