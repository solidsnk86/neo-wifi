...
import { Activity, FileArchive, FileBox, FilePenIcon } from "lucide-react";
      ...
      <section style={{ padding: "2rem" }} className="">
        ...
        <div className="flex flex-col space-y-2 border bg-[#FFFFFF] dark:bg-zinc-800/50 border-zinc-200/70 dark:border-zinc-800 p-4 rounded-2xl relative text-text-primary my-12 w-fit mx-auto">
          ...
          <p className="flex items-center">
            <FileArchive className="mx-2 w-6 h-6" /> {fileInfo?.file}
          </p>
          <p className="flex items-center">
            <FilePenIcon className="mx-2 w-6 h-6" /> Tamaño del fichero:{" "}
            {fileInfo?.size}Mb.
          </p>
          <p className="flex items-center">
          ...
          <p className="flex items-center">
            ...
            {formatDate(fileInfo?.mod || "")}
          </p>
          <aside className="flex justify-end">
            <button
              title={`Descargar ${fileInfo?.file}`}
              className="py-2 px-4 bg-red-500 w-fit mt-4 rounded-xl hover:scale-105 transition-transform duration-300"
              onClick={createLink}
            >
